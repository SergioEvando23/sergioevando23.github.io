...
The `DevSecOps pipeline` promotes one validated static artifact through development, homologation and production: build/tests/coverage, Sonar quality gate, dependency audit, CodeQL, DSV smoke check, HML smoke check, E2E against `E2E_BASE_URL`, then the protected production environment.

## Required GitHub setup

Create `development`, `homologation` and `production` environments. Set `FIREBASE_PROJECT_ID` and `DEPLOY_URL` as environment variables. Store `FIREBASE_SERVICE_ACCOUNT` and `SONAR_TOKEN` only as environment/repository secrets. Configure `SONAR_HOST_URL`, `SONAR_ORGANIZATION` and `SONAR_PROJECT_KEY` as non-secret variables.
