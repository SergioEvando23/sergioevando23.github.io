...
test('carousel supports mouse, indicators, keyboard and mobile layout', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(carousel.getByText('Frontend — React + TypeScript')).toBeVisible();
  ...
  await expect(carousel.getByText('Backend — Node.js + APIs')).toBeVisible();
